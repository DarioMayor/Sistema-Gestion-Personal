...
@admin_bp.route("/admin/fichajes", methods=['GET', 'POST'])
@admin_required
def admin_fichajes():
    conn = mysql.connector.connect(**Config.DB_CONFIG)
    cursor = conn.cursor(dictionary=True)
    ...
    if request.method == 'POST':
        try:
            ...
            return redirect(url_for('admin.admin_fichajes', fecha=fecha_input))
...
# 2. EDITAR FICHAJE
@admin_bp.route("/admin/fichajes/editar/<int:fichaje_id>", methods=['GET', 'POST'])
@admin_required
def editar_fichaje(fichaje_id):
    ...
    if not fichaje:
        ...
        return redirect(url_for('admin.admin_fichajes'))
...
# 3. ELIMINAR FICHAJE
@admin_bp.route("/admin/fichajes/eliminar/<int:fichaje_id>", methods=['POST'])
@admin_required
def eliminar_fichaje(fichaje_id):
    ...
    return redirect(url_for('admin.admin_fichajes', fecha=fecha_retorno))